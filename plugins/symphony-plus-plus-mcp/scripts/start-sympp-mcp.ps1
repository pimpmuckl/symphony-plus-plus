...
. (Join-Path $PSScriptRoot "sympp-mcp-artifact-helpers.ps1")
. (Join-Path $PSScriptRoot "sympp-mcp-runtime-helpers.ps1")
...
$repoRoot = $null
$explicitBackendReuseConfigured = -not [string]::IsNullOrWhiteSpace($env:SYMPP_BACKEND_URL)
$sourceRootExplicitlyConfigured = -not [string]::IsNullOrWhiteSpace($env:SYMPP_REPO_ROOT)
$sourceFallbackAllowed = Test-SourceFallbackAllowed $pluginRoot

if ($sourceRootExplicitlyConfigured) {
...
if ((-not $explicitBackendReuseConfigured) -and
    ($null -ne $artifactProbeError -or $null -eq $artifactProbe -or @("ready", "artifact_selected") -notcontains $artifactProbe.status)) {
  try {
    $repoRoot = Resolve-RepoRoot
    if ([string]::IsNullOrWhiteSpace($expectedSourceRevision)) {
      $expectedSourceRevision = Resolve-SymppSourceRevision $repoRoot $pluginRoot
...
if ($artifactValidationLaunchable -and
    (-not $sourceRootExplicitlyConfigured) -and
    (-not $explicitBackendReuseConfigured) -and
...
if (-not $explicitBackendReuseConfigured -and -not $artifactValidationLaunchable -and $sourceFallbackAllowed -and [string]::IsNullOrWhiteSpace($repoRoot)) {
  $repoRoot = Resolve-RepoRoot
}
