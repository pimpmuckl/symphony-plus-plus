defmodule Mix.Tasks.Sympp.CockpitTest do
  ...
  test "parses safe defaults and explicit options" do
    ...
    try do
      ...
      assert CockpitTask.cockpit_url_for_test(opts, 4567) == "http://localhost:4567/sympp/board"
  ...
  test "starts a local operator cockpit, prints the bound URL, and serves an empty ledger" do
    ...
    try do
      ...
      CockpitTask.run_cockpit_for_test(opts, fn ->
        ...
        send(self(), {:cockpit_response, response.status, response.body})
      end)

      assert_received {:mix_shell, :info, [url]}
      assert url =~ ~r{Symphony\+\+ local operator cockpit: http://127\.0\.0\.1:\d+/sympp/board}
      assert_received {:mix_shell, :info, ["Press Ctrl+C to stop."]}
      assert_received {:cockpit_response, 200, html}
