...
defmodule SymphonyElixir.SymphonyPlusPlus.MCP.CommentsGuidanceTest do
  ...
  test "local operator WorkRequest note tools work from bound worker sessions", %{repo: repo} do
    ...
    out_of_scope_list_response =
      Server.handle(
        %{
          "jsonrpc" => "2.0",
          "id" => "local-operator-bound-list-foreign",
          "method" => "tools/call",
          "params" => %{
            "name" => "list_comments",
            "arguments" => %{"work_package_id" => package.id, "target_kind" => "work_package", "target_id" => other_package.id}
          }
        },
        worker_server
      )
    ...
    stale_bound_list_response =
      Server.handle(
        %{
          "jsonrpc" => "2.0",
          "id" => "local-operator-stale-bound-list-foreign",
          "method" => "tools/call",
          "params" => %{
            "name" => "list_comments",
            "arguments" => %{"work_package_id" => package.id, "target_kind" => "work_package", "target_id" => other_package.id}
        ...
        %{worker_server | session_refresh_required: true}
      )
