defmodule SymphonyElixir.SymphonyPlusPlus.WorkRequestArchitectHandoffTest do
  ...
  test "creates a scoped phase anchor grant and redacted handoff", %{repo: repo, handoff_opts: handoff_opts} do
    ...
    assert handoff.prompt =~ "Do not infer claim state from WorkRequest tool visibility"
    assert handoff.prompt =~ "do not fall back to Solo planning"
    ...
    assert handoff.prompt =~ "valid `source_type` values are: human, architect, operator, ask_pro_advisory"
    assert handoff.prompt =~ "add_work_request_planned_slice"
    assert handoff.prompt =~ "`work_package_kind` must be one of the advertised tool-schema enum values"
