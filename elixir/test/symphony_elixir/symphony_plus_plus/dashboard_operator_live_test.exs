defmodule SymphonyElixir.SymphonyPlusPlus.DashboardOperatorLiveTest do
  ...
  @moduletag skip: "The human-facing operator cockpit is now served by the Vite React shell and operator API tests."
  ...
  test "local operator prepares and replays a WorkRequest architect handoff" do
    ...
    assert html =~ "Tool discovery may already show WorkRequest/architect schemas before claim"
    assert html =~ "WorkRequest tool returns `claim_required`"
    assert html =~ "First scoped MCP reads after binding: `read_work_request`, `list_guidance_requests`"
    assert html =~ "Display key"
    assert html =~ "Do not infer claim state from WorkRequest tool visibility"
    assert html =~ "do not fall back to Solo planning"
