...
defmodule SymphonyElixir.SymphonyPlusPlus.MCPTest do
  ...
  test "child worker key minting ignores normal worker grants during replacement", %{repo: repo} do
    ...
    assert get_in(second_response, ["result", "structuredContent", "superseded_child_handoff_cleanup", "status"]) == "cleaned"
  ...
  test "child worker key minting supersedes unclaimed grants and rejects claimed active grants", %{repo: repo} do
    ...
    assert get_in(second_response, ["result", "structuredContent", "superseded_child_handoff_cleanup", "status"]) == "cleaned"
    assert get_in(second_response, ["result", "structuredContent", "superseded_child_handoff_cleanup", "attempted"]) == 1
    ...
    _worker_session = claim_child_worker_from_mint_response(repo, second_response, "worker-1")
  ...
  test "child worker key minting revokes the new grant when private handoff storage fails", %{repo: repo} do
    {_anchor, architect_session} =
    ...
    assert length(child_delegated_grants) == 1
    assert [%AccessGrant{revoked_at: %DateTime{}}] = child_delegated_grants
    assert active_worker_grants(grants) == []
  end
