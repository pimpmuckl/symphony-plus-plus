defmodule SymphonyElixir.SymphonyPlusPlus.WorkRequests.ArchitectHandoff do
  ...
  defp prompt(%WorkRequest{} = work_request, %Phase{} = phase, %WorkPackage{} = anchor, redacted_handoff, handoff_opts)
    ...
    [
      ...
      "First MCP step: bind this session through the private handoff. Tool discovery may already show WorkRequest/architect schemas before claim; schema visibility is not authorization. If a WorkRequest tool returns `claim_required`, use the handoff bootstrap/`claim_work_key` recovery path first, then retry after binding.",
      ...
      "2. Do not infer claim state from WorkRequest tool visibility. If the session is unbound or a WorkRequest tool returns `claim_required`, do not fall back to Solo planning; use the handoff bootstrap/`claim_work_key` recovery path to bind the architect grant, then retry the scoped reads.",
