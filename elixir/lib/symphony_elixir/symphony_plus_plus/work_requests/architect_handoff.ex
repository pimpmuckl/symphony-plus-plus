defmodule SymphonyElixir.SymphonyPlusPlus.WorkRequests.ArchitectHandoff do
  ...
  defp prompt(%WorkRequest{} = work_request, %Phase{} = phase, %WorkPackage{} = anchor, redacted_handoff, handoff_opts)
    ...
    [
      ...
      "First MCP step: bind this session through the private handoff. If tool discovery shows only `sympp.health`, Solo Session tools, and `claim_work_key`, you are pre-claim; use the handoff bootstrap first, then refresh tool discovery.",
      ...
      "2. If the WorkRequest tools are not visible yet, do not fall back to Solo planning. Use the handoff bootstrap/`claim_work_key` recovery path to bind the architect grant, then refresh tool discovery.",
