defmodule SymphonyElixir.SymphonyPlusPlus.SecretHandoff do
  ...
  def error_message(:unsupported_handoff_metadata_dir), do: "secret handoff metadata must use the managed metadata directory"
  ...
  defp handoff_metadata_context(%WorkPackage{} = work_package, worker_grant, opts) do
    with :ok <- reject_metadata_dir_override(opts),
      ...
      metadata_filename =
        "handoff-#{handoff_metadata_hash(work_package.id, display_key, grant_identity, opts)}.json"
      ...
      expected_local_private_file_path =
        local_private_file_path(work_package, %{id: grant_identity, display_key: display_key}, opts)
      ...
      {:ok,
       %{
         ...
         expected_local_private_file_path: expected_local_private_file_path
       }}
    end
  end
  ...
  defp handoff_metadata_hash(work_package_id, display_key, grant_identity, opts) do
    hash_source = [
      "v1",
      0,
      to_string(Keyword.get(opts, :repo_root, "")),
  ...
  defp handoff_metadata_record(context, handoff) do
    with {:ok, mode} <- handoff_metadata_mode(handoff),
      ...
      metadata =
        ...
        |> Map.merge(Map.put(coordinates, "mode", mode))
  ...
  defp handoff_metadata_coordinates("local-private-file", context, handoff) do
    case handoff_value(handoff, :path) do
      path when is_binary(path) ->
        validate_handoff_metadata_local_path(path, context)
  ...
  defp handoff_metadata_coordinates("windows-credential-manager", context, handoff) do
    expected_target =
      credential_target(context.work_package, %{id: context.grant_identity, display_key: context.display_key})
  ...
  defp validate_handoff_metadata_local_path(path, context) do
    if String.trim(path) == "" do
      ...
      expected_path = context.expected_local_private_file_path

      cond do
        path != expected_path -> {:error, {:handoff_metadata_invalid, :local_path_mismatch}}
        File.regular?(path) -> {:ok, %{"path" => path}}
        true -> {:error, {:handoff_metadata_invalid, :missing_local_file}}
      end
    end
  end
  ...
  defp allow_equivalent_handoff_metadata_write(path, encoded) do
    with {:ok, existing} <- File.read(path),
      ...
      if existing_metadata == new_metadata do
        ...
        {:error, :handoff_metadata_conflict}
      end
    else
